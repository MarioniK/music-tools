...
def test_tensorflow_input_musiccnn_probe_report_is_validated():
    validator = load_validator()

    validator._validate_musicnn_tensorflow_input_musiccnn_availability_probe_report(REPORT_PATH)


def test_tensorflow_input_musiccnn_probe_report_records_execution_boundary():
    data = json.loads(REPORT_PATH.read_text(encoding="utf-8"))

    assert data["isolated_python_used_for_availability_check"] is True
    assert data["preprocessing_probe_execution"] == "current_interpreter"
    assert "approved isolated Python interpreter" in data["recommended_isolated_probe_invocation"]
    assert "--mode preprocessing-probe" in data["recommended_isolated_probe_invocation"]


def test_tensorflow_input_musiccnn_probe_report_rejects_private_paths(tmp_path):
    validator = load_validator()
    data = json.loads(REPORT_PATH.read_text(encoding="utf-8"))
